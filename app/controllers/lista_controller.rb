class ListaController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @listum.save
        format.html { redirect_to listum_url(@listum), notice: "Listum was successfully created." }
  ...
  def update
    respond_to do |format|
      if @listum.update(listum_params)
        format.html { redirect_to listum_url(@listum), notice: "Listum was successfully updated." }
  ...
  private
    ...
    def listum_params
      params.require(:listum).permit(:nome, :tarefa, :status, :categoria, :data_inicio, :data_conclusao)
    end
end
